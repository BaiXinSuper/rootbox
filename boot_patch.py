...
class Patch:
    def __init__(self, add_text,keepverity=True, keepforceencrypt=True, patchvbmetaflag=False, recoverymode=False, debug=False):
        # init defaults
    ...
    def execv(self, cmd, verbose=False):
        ...
        try:
            ...
            self.add_text("! Cannot execute program\n")
            return -1
        if self.debug:
            self.add_text(ret.stdout.decode('utf-8'))
    ...
    def patchboot(self, infile):
        ...
        # unpack boot image
        self.add_text("- Unpacking boot image\n")
        retcode = self.execv([self.magiskboot, "unpack", infile])
        if retcode == 0:
            pass
        elif retcode == 1:
            self.add_text("! Unsupported/Unknown image format\n")
            return False
        elif retcode == 2:
            self.add_text("- ChromeOS boot image detected\n")
            self.CHROMEOS = True
        else:
            self.add_text("! Unable to unpack boot image\n")
            return False
        
        # check ramdisk status
        self.add_text("- Checking ramdisk status\n")
        ...
        status = retcode
        if (status & 3) == 0:
            self.add_text("- Stock boot image detected\n")
            SHA1 = self.exegetout([self.magiskboot, "sha1", "%s" %infile])
            shutil.copyfile(infile, "stock-boot.img")
        elif (status & 3) == 1:
            self.add_text("- Magisk patched boot image detected\n")
            if os.getenv("SHA1") == "":
            ...
            self.add_text("! Boot image patched by unsupported programs\n",
        ...
        # Ramdisk Patches
        self.add_text("- Patching ramdisk\n")
        ...
        for dt in ["dtb", "kernel_dtb", "extra"]:
            if os.path.isfile(dt):
                if not self.execv([self.magiskboot, "dtb", dt, "test"]) == 0:
                    self.add_text("! Boot image %s was patched by old (unsupported) Magisk\n" %dt,
                                     "! Please try again with *unpatched* boot image\n")
                    return False
                if self.execv([self.magiskboot, "dtb", dt, "patch"]) == 0:
                    self.add_text("- Patch fstab in boot image %s dt" %dt)
        ...
        # Repack & Flash
        self.add_text("- Repacking boot image\n")
        if not self.execv([self.magiskboot, "repack", infile]) == 0:
            self.add_text("! Unable to repack boot image\n")
            return False
        
        if self.CHROMEOS:
            self.add_text("- Not support sign with futility on python yet...")
            return False
